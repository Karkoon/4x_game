...
public class GameScreenInputAdapter extends InputAdapter {
  ...
  private static class GameObject {
    ...
    private GameObject(Coordinates coords) {
      ...
      radius = 100f; // i radius skądś powiązać z tym co zostało zrobione w PositionUtil jako no te wartości
      // ale na razie jest zawsze taki sam więc nie trzeba go liczyć
    }
  }
  ...
  public boolean touchDown (int screenX, int screenY, int pointer, int button) {
    var selecting = getObject(screenX, screenY);
    if (selecting != null) {
